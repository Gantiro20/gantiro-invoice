            ...
            {user && (
                <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] z-40">
                    <nav className="flex justify-around items-center h-16 max-w-3xl mx-auto">
                        <NavButton icon="post_add" label="فاکتور جدید" active={window.location.hash === '#create'} onClick={() => window.location.hash = '#create'} />
                        
                        {canViewHistory && (
                            <NavButton icon="history" label="تاریخچه" active={window.location.hash === '#dashboard'} onClick={() => window.location.hash = '#dashboard'} />
                        )}
                        
                        {user.role === 'admin' && (
                            <NavButton icon="admin_panel_settings" label="مدیریت" active={window.location.hash === '#admin'} onClick={() => window.location.hash = '#admin'} />
                        )}
                    </nav>
                </div>
            )}
